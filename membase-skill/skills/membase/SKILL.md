...
All Membase operations go through a single command. Run from the skill install directory (e.g. `membase-skill/skills/membase`):

```bash
# Option 1: OpenClaw (if plugin installed)
openclaw membase <command> [options]

# Option 2: npx tsx (TypeScript)
npx tsx membase.ts <command> [options]

# Option 3: bun
bun membase.ts <command> [options]
```

Ensure `npm install` and `npm run build` have been run at the package root.
...
**Usage:**
```bash
npx tsx membase.ts backup [options]
# or: openclaw membase backup [options]
```
...
3. Run backup (from skill directory):
   ```bash
   cd skills/membase
   npx tsx membase.ts backup --password "<password>"
   ```
...
```bash
npx tsx membase.ts backup --password "<password>" --incremental
```
...
**Usage:**
```bash
npx tsx membase.ts restore <backup-id> [options]
```
...
2. Run restore:
   ```bash
   cd skills/membase
   npx tsx membase.ts restore backup-2026-02-02T10-30-45-123Z --password "<password>"
   ```
...
**Usage:**
```bash
npx tsx membase.ts list [options]
```
...
**Usage:**
```bash
npx tsx membase.ts diff <backup-id-1> <backup-id-2> [options]
```
...
2. Run diff with the two IDs:
   ```bash
   npx tsx membase.ts diff backup-2026-02-02T10-30-45-123Z backup-2026-02-01T15-20-10-456Z --password "<password>"
   ```
...
**Usage:**
```bash
npx tsx membase.ts status [options]
```
...
**Usage:**
```bash
npx tsx membase.ts cleanup [options]
```
...
```bash
cd skills/membase
npx tsx membase.ts cleanup --keep-last 5
```
...
# 2. First backup (full)
npx tsx membase.ts backup --password "MySecure123Pass"

# 3. Later: incremental backup
npx tsx membase.ts backup --password "MySecure123Pass" --incremental

# 4. List all backups
npx tsx membase.ts list

# 5. Compare recent backups
npx tsx membase.ts diff backup-id-1 backup-id-2 --password "MySecure123Pass"

# 6. Restore if needed
npx tsx membase.ts restore backup-id-1 --password "MySecure123Pass"
```
